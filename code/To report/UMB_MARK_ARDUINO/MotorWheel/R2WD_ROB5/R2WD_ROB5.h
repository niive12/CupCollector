...
class R2WD_ROB5 {
public:
	R2WD_ROB5(MotorWheel* wheelLeft,MotorWheel* wheelRight,unsigned int wheelspanMM=WHEELSPAN);
    ...
    // Get time (Mikkel)
    unsigned long getRotationTime(unsigned int speedMMPS, float radian);
    unsigned long getStraightTime(unsigned int speedMMPS, unsigned long distance);
