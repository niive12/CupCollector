...
struct pos_t : pair<coordIndexType, coordIndexType> {
	...
	/** Small vector operations can be performed with the following operators */
...
public:
	...
	/**
	 * @brief tekMap Default constructor (doing nothing)
	 */
	tekMap();
...
template<typename coordValType>
void tekMap<coordValType>::convert_into_pgm( shared_ptr<Image> img, const std::string& fileName )
{
	unsigned char val;
...
template<typename coordValType>
tekMap<coordValType>::tekMap(shared_ptr<Image> img)
{
	if( !img )
...
template<typename coordValType>
tekMap<coordValType>::tekMap() {}

template<typename coordValType>
void tekMap<coordValType>::shade(shared_ptr<Image> img) const
{
...
template<typename coordValType>
list<pos_t> tekMap<coordValType>::getOffloadingStations(const shared_ptr<Image> img)
{
	list<pos_t> result;
